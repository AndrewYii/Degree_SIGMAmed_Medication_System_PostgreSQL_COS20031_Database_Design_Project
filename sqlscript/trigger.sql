    ...
    IF OLD."Role" != NEW."Role" THEN
        ...
        ELSIF NEW."Role" = 'patient' THEN
            INSERT INTO "SIGMAmed"."Patient" (
                ...
                NEW."UserId", 'TEMP-' || NEW."UserId", '[]'::JSONB
            );
...
--         PERFORM pg_advisory_xact_lock(hashtext('prescription_number_seq_' || v_year || v_month));
...
--         WHERE "PrescriptionNumber" ~ ('^RX-' || v_year || v_month || '-[0-9]{5}$');
        
--         v_prescription_number := 'RX-' || v_year || v_month || '-' || LPAD(v_counter::TEXT, 5, '0');
